...
void EgoInfo::CalculateFrontObstacleClearDistance(const std::vector<const Obstacle*>& obstacles) {
    ...
    Box2d ego_front_region(center + unit_vec_heading * kDistanceThreshold / 2.0, vehicle_state_.heading(),
                           impact_region_length, param.width() + buffer);
    ...
    for (const auto& obstacle : obstacles) {
        ...
        double dist =
            ego_box_.center().DistanceTo(obstacle->PerceptionBoundingBox().center()) - ego_box_.diagonal() / 2.0;
