...
bool LatController::LoadControlConf() {
    ...
    // preview_window_ 默认为0
    preview_window_ = lat_based_lqr_controller_conf_.preview_window();
    lookahead_station_low_speed_ = lat_based_lqr_controller_conf_.lookahead_station();
...
void LatController::ProcessLogs(const SimpleLateralDebug* debug, const canbus::Chassis* chassis) {
    const std::string log_str = absl::StrCat(
        debug->lateral_error(), ",", debug->ref_heading(), ",", debug->heading(), ",", debug->heading_error(), ",",
        debug->heading_error_rate(), ",", debug->lateral_error_rate(), ",", debug->curvature(), ",",
        debug->steer_angle(), ",", debug->steer_angle_feedforward(), ",", debug->steer_angle_lateral_contribution(),
        ",", debug->steer_angle_lateral_rate_contribution(), ",", debug->steer_angle_heading_contribution(), ",",
...
void LatController::InitializeFilters() {
    ...
    common::LpfCoefficients(ts_, lat_based_lqr_controller_conf_.cutoff_freq(), &den, &num);
    digital_filter_.set_coefficients(den, num);
    lateral_error_filter_ =
        common::MeanFilter(static_cast<std::uint_fast8_t>(lat_based_lqr_controller_conf_.mean_filter_window_size()));
...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector) {
    if (!ControlTask::LoadConfig<LatBaseLqrControllerConf>(&lat_based_lqr_controller_conf_)) {
        ...
        return Status(ErrorCode::CONTROL_INIT_ERROR, "failed to load lat control_conf");
    }
    injector_ = injector;
    if (!LoadControlConf()) {
        AERROR << "failed to load control conf";
    ...
    */
    ...
    if (enable_mrac_) {
        mrac_controller_.Init(lat_based_lqr_controller_conf_.steer_mrac_conf(), vehicle_param_.steering_latency_param(),
                              ts_);
    }
    // enable_look_ahead_back_control 配置中为true
...
void LatController::LoadLatGainScheduler() {
    const auto& lat_err_gain_scheduler = lat_based_lqr_controller_conf_.lat_err_gain_scheduler();
    const auto& heading_err_gain_scheduler = lat_based_lqr_controller_conf_.heading_err_gain_scheduler();
...
Status LatController::ComputeControlCommand(const localization::LocalizationEstimate* localization,
    ...
    // enable_navigation_mode_position_update 配置中为true
    if (FLAGS_use_navigation_mode && lat_based_lqr_controller_conf_.enable_navigation_mode_position_update()) {
    ...
    // 如果条件匹配，将规划轨迹的坐标从后轴中心变换到质心
    // trajectory_transform_to_com_reverse 配置中为true
    // trajectory_transform_to_com_drive 配置中为true
    ...
    */
    ...
    if (FLAGS_enable_gain_scheduler) {
        matrix_q_updated_(0, 0) =
            matrix_q_(0, 0) * lat_err_interpolation_->Interpolate(std::fabs(vehicle_state->linear_velocity()));
        matrix_q_updated_(2, 2) =
            matrix_q_(2, 2) * heading_err_interpolation_->Interpolate(std::fabs(vehicle_state->linear_velocity()));
        common::math::SolveLQRProblem(matrix_adc_, matrix_bdc_, matrix_q_updated_, matrix_r_, lqr_eps_,
                                      lqr_max_iteration_, &matrix_k_, &num_iteration, &result_diff);
    } else {
        common::math::SolveLQRProblem(matrix_adc_, matrix_bdc_, matrix_q_, matrix_r_, lqr_eps_, lqr_max_iteration_,
        ...
        if (lat_based_lqr_controller_conf_.enable_feedback_augment_on_high_speed() ||
            ...
            if (std::fabs(vehicle_state->linear_velocity()) > low_speed_bound_ - low_speed_window_) {
                ...
                steer_angle_feedback_augment =
                    common::math::lerp(steer_angle_feedback_augment, low_speed_bound_ - low_speed_window_, 0.0,
                                       low_speed_bound_, std::fabs(vehicle_state->linear_velocity()));
            }
        }
    }
    ...
    cmd->set_steering_target(common::math::Clamp(steer_angle, pre_steer_angle_ - steer_diff_with_max_rate,
    ...
    const double steer_angle_lateral_contribution =
        -matrix_k_(0, 0) * matrix_state_(0, 0) * 180 / M_PI * steer_ratio_ / steer_single_direction_max_degree_ * 100;

    const double steer_angle_lateral_rate_contribution =
        -matrix_k_(0, 1) * matrix_state_(1, 0) * 180 / M_PI * steer_ratio_ / steer_single_direction_max_degree_ * 100;

    const double steer_angle_heading_contribution =
        -matrix_k_(0, 2) * matrix_state_(2, 0) * 180 / M_PI * steer_ratio_ / steer_single_direction_max_degree_ * 100;

    const double steer_angle_heading_rate_contribution =
        -matrix_k_(0, 3) * matrix_state_(3, 0) * 180 / M_PI * steer_ratio_ / steer_single_direction_max_degree_ * 100;
...
void LatController::UpdateState(SimpleLateralDebug* debug, const canbus::Chassis* chassis) {
    auto vehicle_state = injector_->vehicle_state();
...
void LatController::UpdateMatrixCompound() {
    ...
    // 从0 0开始，取一个大小为 basic_state_size_ * basic_state_size_ 的子矩阵，将
    // matrix_ad_ 的值赋值给这个子矩阵
...
double LatController::ComputeFeedForward(double ref_curvature) const {
    ...
    if (injector_->vehicle_state()->gear() == canbus::Chassis::GEAR_REVERSE &&
        !lat_based_lqr_controller_conf_.reverse_use_dynamic_model()) {
        steer_angle_feedforwardterm = lat_based_lqr_controller_conf_.reverse_feedforward_ratio() * wheelbase_ *
                                      ref_curvature * 180 / M_PI * steer_ratio_ / steer_single_direction_max_degree_ *
                                      100;
    } else {
...
void LatController::ComputeLateralErrors(const double x, const double y, const double theta, const double linear_v,
                                         const double angular_v, const double linear_a,
                                         const TrajectoryAnalyzer& trajectory_analyzer, SimpleLateralDebug* debug,
                                         const canbus::Chassis* chassis) {
    TrajectoryPoint target_point;

    // query_time_nearest_point_only 默认为false
    ...
    /*
      vec(curr2ref) = [ref.x - curr.x, ref.y - curr.y]          当前点指向参考点的向量
      vec(ref_tangent_unit) = [cos(ref.theta), sin(ref.theta)]  参考点的单位切向量
      vec(ref_nomal_unit) = [-sin(ref.theta), cos(ref.theta)]   参考点的单位法向量
      lateral_error = vec(curr2ref) · vec(ref_nomal_unit)
     */
    ...
    if (std::fabs(linear_v) >= low_speed_bound_) {
        ...
        lookahead_station = common::math::lerp(lookahead_station_low_speed_, low_speed_bound_ - low_speed_window_,
                                               lookahead_station_high_speed_, low_speed_bound_, std::fabs(linear_v));
        lookback_station = common::math::lerp(lookback_station_low_speed_, low_speed_bound_ - low_speed_window_,
