...
class LatController : public ControlTask {
   ...
   protected:
    ...
    void ComputeLateralErrors(const double x, const double y, const double theta, const double linear_v,
    ...
    Eigen::MatrixXd matrix_a_;  // 车辆状态矩阵
    // vehicle state matrix (discrete-time)
    Eigen::MatrixXd matrix_ad_;  // 离散时间的车辆状态矩阵
    // vehicle state matrix compound; related to preview
    Eigen::MatrixXd matrix_adc_;  // 复合矩阵，一般用来一次性把多个采样周期的状态叠加起来
    // control matrix
...
// 1.2 当前类声明为插件
CYBER_PLUGIN_MANAGER_REGISTER_PLUGIN(apollo::control::LatController, ControlTask)
