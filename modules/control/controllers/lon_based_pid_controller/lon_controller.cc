...
void LonController::InitControlCalibrationTable() {
    AINFO << "Control calibration table size is " << calibration_table_.calibration_size();
...
Status LonController::ComputeControlCommand(const localization::LocalizationEstimate *localization,
    ...
    //配置参数参考 modules/control/controllers/lon_based_pid_controller/conf/controller_conf.pb.txt

    //纵向控制器的采样时间(好像是控制周期，pid为0.01)
    double ts = lon_based_pidcontroller_conf_.ts();
    // preview_window: 预瞄时间窗口(控制器前瞻一段路径进行计算，pid为20)
    ...
    // enable_speed_station_preview 默认为false
    if (lon_based_pidcontroller_conf_.enable_speed_station_preview()) {
        // preview_station_error 基于预瞄时间查找的预瞄点和当前轨迹上距离车辆最近点的曲线长度差值
        // station_error_limited 好像是位置误差
        station_error_limited =
            common::math::Clamp(debug->preview_station_error(), -station_error_limit, station_error_limit);
    } else {
        // station_error 基于当前时间查找的参考点的曲线长度减去轨迹上距离车辆最近点的曲线长度
    ...
    if (trajectory_message_->gear() == canbus::Chassis::GEAR_REVERSE) {
        if (CheckPit::CheckInPit(debug, &lon_based_pidcontroller_conf_, injector_->vehicle_state()->linear_velocity(),
        ...
        if (enable_leadlag) {
            ...
            speed_leadlag_controller_.SetLeadlag(lon_based_pidcontroller_conf_.reverse_speed_leadlag_conf());
               ...
               lon_based_pidcontroller_conf_.switch_speed()) {  // switch_speed 高低速控制器切换的阈值
        if (CheckPit::CheckInPit(debug, &lon_based_pidcontroller_conf_, injector_->vehicle_state()->linear_velocity(),
                                 trajectory_message_->is_replan())) {
            ADEBUG << "in pit";
            station_pid_controller_.SetPID(lon_based_pidcontroller_conf_.pit_station_pid_conf());
    ...
    // speed_controller_input_limit 默认1.5
    ...
    if (lon_based_pidcontroller_conf_.enable_speed_station_preview()) {
        ...
        // speed_error 基于当前时间查找的参考点的速度减去轨迹上距离车辆最近点的s的导数
    ...
    // speed_controller_input_limited 应该是速度误差
    ...
    double slope_offset_compensation = lon_based_pidcontroller_conf_.use_opposite_slope_compensation() * GRA_ACC *
    ...
    if (!previous_full_stop) {
        ...
        max_path_remain_when_stopped_ =
            FLAGS_max_path_remain_when_stopped + lon_based_pidcontroller_conf_.full_stop_path_remain_gain();
    }
    ...
    if (debug->is_full_stop()) {
        acceleration_cmd = (chassis->gear_location() == canbus::Chassis::GEAR_REVERSE)
                               ? std::max(acceleration_cmd, -lon_based_pidcontroller_conf_.standstill_acceleration())
                               : std::min(acceleration_cmd, lon_based_pidcontroller_conf_.standstill_acceleration());
        speed_pid_controller_.Reset_integral();
        station_pid_controller_.Reset_integral();
    }
    ...
    double throttle_lowerbound =
        std::max(vehicle_param_.throttle_deadzone(), lon_based_pidcontroller_conf_.throttle_minimum_action());
    double brake_lowerbound =
        std::max(vehicle_param_.brake_deadzone(), lon_based_pidcontroller_conf_.brake_minimum_action());
    ...
    if (lon_based_pidcontroller_conf_.use_acceleration_lookup_limit()) {
        acceleration_lookup_limit =
            (acceleration_lookup > acceleration_lookup_limited) ? acceleration_lookup_limited : acceleration_lookup;
    }

    if (FLAGS_use_preview_speed_for_table) {
        if (lon_based_pidcontroller_conf_.use_acceleration_lookup_limit()) {
            calibration_value = control_interpolation_->Interpolate(
                std::make_pair(std::fabs(debug->preview_speed_reference()), acceleration_lookup_limit));
        } else {
            calibration_value = control_interpolation_->Interpolate(
                std::make_pair(std::fabs(debug->preview_speed_reference()), acceleration_lookup));
        }
    } else {
        if (lon_based_pidcontroller_conf_.use_acceleration_lookup_limit()) {
            calibration_value = control_interpolation_->Interpolate(
                std::make_pair(std::fabs(injector_->vehicle_state()->linear_velocity()), acceleration_lookup_limit));
        } else {
            calibration_value = control_interpolation_->Interpolate(
                std::make_pair(std::fabs(injector_->vehicle_state()->linear_velocity()), acceleration_lookup));
        }
    }
    ...
    if (FLAGS_enable_csv_debug && speed_log_file_ != nullptr) {
        fprintf(speed_log_file_,
                ...
                speed_controller_input_limited, debug->preview_speed_reference(), debug->preview_speed_error(),
                debug->preview_acceleration_reference(), acceleration_cmd_closeloop, acceleration_cmd,
                debug->acceleration_lookup(), debug->acceleration_lookup_limit(), debug->speed_lookup(),
    ...
    if (lon_based_pidcontroller_conf_.use_speed_itfc()) {
        ...
        if ((reference_spd_ <= lon_based_pidcontroller_conf_.speed_itfc_full_stop_speed()) &&
            (chassis->gear_location() == canbus::Chassis::GEAR_DRIVE)) {
            if ((debug->path_remain() >= lon_based_pidcontroller_conf_.speed_itfc_path_remain_min()) &&
                (debug->preview_acceleration_reference() >= lon_based_pidcontroller_conf_.speed_itfc_dcc_emergency()) &&
                (debug->path_remain() <= lon_based_pidcontroller_conf_.speed_itfc_path_remain_max())) {
                if (debug->preview_acceleration_reference() <= lon_based_pidcontroller_conf_.speed_itfc_acc_thres()) {
                    reference_spd_cmd_ = lon_based_pidcontroller_conf_.speed_itfc_speed_cmd();
                }
            }
        }
...
void LonController::ComputeLongitudinalErrors(const TrajectoryAnalyzer *trajectory_analyzer, const double preview_time,
    ...
    double one_minus_kappa_lat_error =
        1 - reference_point.path_point().kappa() * vehicle_state->linear_velocity() * std::sin(heading_error);
    ...
    if (trajectory_message_->gear() == canbus::Chassis::GEAR_DRIVE) {
        ...
        while (stop_index < trajectory_message_->trajectory_point_size()) {
            ...
            if (current_trajectory_point.v() > -kSpeedThreshold &&
                current_trajectory_point.a() < kBackwardAccThreshold && current_trajectory_point.a() > 0.0) {
                break;
            }
            ++stop_index;
        }
...
bool LonController::IsFullStopLongTerm(SimpleLongitudinalDebug *debug) {
    if (debug->is_full_stop()) {
        if (debug->is_full_stop() && !is_full_stop_previous_) {
            ...
            ADEBUG << "Full stop long term start time(s) is " << is_full_stop_start_time_;
        } else {
    ...
    if (is_full_stop_wait_time_diff_ > lon_based_pidcontroller_conf_.full_stop_long_time()) {
        ADEBUG << "Current full stop lasting time(s) is " << is_full_stop_wait_time_diff_
               << ", larger than threshold: " << lon_based_pidcontroller_conf_.full_stop_long_time();
        return true;
    } else {
        ADEBUG << "Current full stop lasting time(s) is " << is_full_stop_wait_time_diff_
               << ", not reach the threshold: " << lon_based_pidcontroller_conf_.full_stop_long_time();
        return false;
    }
}

void LonController::SetParkingBrake(const LonBasedPidControllerConf *conf, control::ControlCommand *control_command) {
