...
ControlComponent::ControlComponent() : monitor_logger_buffer_(common::monitor::MonitorMessageItem::CONTROL) {}
...
bool ControlComponent::Init() {
    ...
    AINFO << "ControlTask pipeline config file: " << FLAGS_pipeline_file << " is loaded.";
    ...
    if (!FLAGS_is_control_ut_test_mode) {
        if (!FLAGS_use_control_submodules && !control_task_agent_.Init(injector_, control_pipeline_).ok()) {
            // set controller
    ...
    planning_command_status_reader_ =
        node_->CreateReader<external_command::CommandStatus>(planning_command_status_reader_config, nullptr);
    ...
    AINFO << "Control default driving action is " << DrivingAction_Name((enum DrivingAction)FLAGS_action);
...
//把规划出的轨迹存储到 latest_trajectory_ 中
...
void ControlComponent::OnPlanningCommandStatus(
    const std::shared_ptr<external_command::CommandStatus> &planning_command_status) {
...
Status ControlComponent::ProduceControlCommand(ControlCommand *control_command) {
    ...
    // check estop
    estop_ = FLAGS_enable_persistent_estop ? estop_ || local_view_.trajectory().estop().is_estop()
    ...
    if (!local_view_.trajectory().trajectory_point().empty()) {
        ...
        Status status_compute = control_task_agent_.ComputeControlCommand(
            &local_view_.localization(), &local_view_.chassis(), &local_view_.trajectory(), control_command);
...
bool ControlComponent::Proc() {
    ...
    injector_->mutable_control_debug_info()->mutable_control_component_debug()->Clear();
    ...
    if (fabs(control_command.debug().simple_lon_debug().vehicle_pitch()) < kDoubleEpsilon) {
        injector_->vehicle_state()->Update(local_view_.localization(), local_view_.chassis());
    ...
    const double process_control_time_diff = (end_process_control_time - start_time).ToSecond() * 1e3;
    if (control_command.mutable_latency_stats()->total_time_exceeded()) {
        AINFO << "control all spend time is: " << process_control_time_diff << " ms.";
    }
...
Status ControlComponent::CheckInput(LocalView *local_view) {
    ...
    if (!local_view->trajectory().estop().is_estop() && local_view->trajectory().trajectory_point().empty()) {
        AWARN_EVERY(100) << "planning has no trajectory point. ";
...
void ControlComponent::GetVehiclePitchAngle(ControlCommand *control_command) {
    ...
    control_command->mutable_debug()->mutable_simple_lon_debug()->set_vehicle_pitch(vehicle_pitch +
                                                                                    FLAGS_pitch_offset_deg);
}

void ControlComponent::CheckAutoMode(const canbus::Chassis *chassis) {
    if (!injector_->previous_control_debug_mutable()->mutable_control_component_debug()->is_auto() &&
    ...
    injector_->mutable_control_debug_info()->mutable_control_component_debug()->set_from_else_to_auto(
        from_else_to_auto_);
    ...
    injector_->mutable_control_debug_info()->mutable_control_component_debug()->set_is_auto(is_auto_);
}
...
void ControlComponent::PublishControlInteractiveMsg() {
    ...
    ADEBUG << "control interactive msg is: " << control_interactive_msg.ShortDebugString();
