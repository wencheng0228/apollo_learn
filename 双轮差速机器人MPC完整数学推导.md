...
| **全变量形式（本文）** | 状态+控制 $\boldsymbol{\xi} = [\boldsymbol{\Xi}, \mathbf{U}]$ | 通过等式约束保证 | 易实现，易约束，Apollo采用 |
...
全变量形式的核心：通过**等式约束**保证状态和控制满足动力学关系。
...
- 全变量形式中，初始状态通过等式约束引入，目标函数中不含一次项
- 这使得全变量形式的构造更加简洁
...
在全变量形式中，决策变量 $\boldsymbol{\xi} = [\boldsymbol{\Xi}, \mathbf{U}]$，状态变量占据前30个元素。
...
在全变量形式中，控制变量占据决策变量的后20个元素（索引30-49）。
